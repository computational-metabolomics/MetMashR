...
kegg_lookup = function(
    ...
    if (get %in% c('compound','drug','glycan')  & 
        ...
        stop('compound, drug and glycan ids can only be converted to chebi ',
            'or pubchem ids')
    }
    if (get %in% c('pubchem','chebi')  & 
            from %in% c('pubchem','chebi')) {
        stop('pubchem and chebi ids can only be converted to compound, drug ',
            'or glycan ids')
    }
...
.kegg_lookup<-setClass(
    ...
    prototype=list(
        ...
        query_column = entity(
            ...
            description = paste0('The name of the column containing ',
