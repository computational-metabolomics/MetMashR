...
setMethod(f = "read_source",
          ...
          definition = function(obj) {
              ...
              L = list()
              L[[1]] = left_join(TB1$blue,TB1$orange,by = join_by(blue_id),suffix=c('.blue','.orange'))
              L[[2]] = left_join(TB2$blue,TB2$orange,by = join_by(blue_id),suffix=c('.blue','.orange'))
              
              # join grey
              L[[1]] = left_join(L[[1]],TB1$grey,by = join_by(
                  blue_id == blue_id,
                  orange_id == orange_id),suffix=c('.blueorange','.grey'))
              
              # summarise
              L[[1]] = L[[1]] %>% group_by(blue_id,Ion) %>% summarise(
                  Compound = unique(Name),
                  Formula       = unique(Formula),
                  RT            = mean(as.numeric(.data[['RT [min].grey']])*60,na.rm = TRUE),
                  mzcloud_score = mean(as.numeric(mzCloud.Best.Match),na.rm = TRUE),
              ...
              OUT = left_join(L[[1]],L[[2]],by = join_by(blue_id),
...
.read_cd_isomers_file = function(M,idx){
    ...
    for (k in 1:nrow(cd)) {
        n = sum(cd$Checked[1:k] %in% c('TRUE','FALSE'),na.rm = TRUE)
        cd$blue_id[k] = n
        n = sum(cd$Name[1:k] %in% c('TRUE','FALSE'),na.rm = TRUE)
        cd$orange_id[k]  = n
    }
    ...
    # filter
    orange = cd %>% select(all_of(x)) %>% filter(Checked %in% c('TRUE','FALSE'))
...
.read_cd_compounds_file = function(M,idx) {
    ...
    ## blue rows
    blue = cd %>% filter(Checked=="FALSE") %>% select(-orange_id)
    ...
    x = setNames(names(x), x)
    # filter
    grey = cd %>% select(all_of(x)) %>% filter(Checked=='FALSE')
