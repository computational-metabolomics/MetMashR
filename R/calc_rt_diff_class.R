...
.calc_rt_diff<-setClass(
    ...
    prototype=list(
        ...
        type = 'univariate',
        predicted = 'updated',
        .params=c('obs_rt_column','ref_rt_column','out_column','check_names'),
        .outputs=c('updated'),
        updated=entity(
        ...
        obs_rt_column=entity(
            name='Observed m/z',
            description = 'Column name in annotation table containing the observed (measured) RT values.',
            type='character'
        ),
        ref_rt_column=entity(
            name='Reference m/z',
            description = 'Column name in annotation table containing the reference (theoretical) RT values.',
            type='character'
        ),
        out_column=entity(
            name='Outout column',
            description = 'Column name in annotation table to store the computed RT differences.',
            type='character'
        ),
        check_names=enum(
            name='Check names',
...
setMethod(f="model_apply",
    ...
    definition=function(M,D) {
        ...
        if (M$check_names=='stop' & colname_check) {
            stop('There is already a column called ', M$out_column, 
