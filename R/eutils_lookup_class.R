...
.eutils_lookup<-setClass(
    ...
    prototype=list(
        ...
        url_template = entity(
            ...
            value = c(
                '<base_url>/esearch.fcgi?db=<database>&term="<query_column>"<term>&retmode=json'
            ),
            max_length = 1
        ),
        ...
        term = entity(
            ...
            description = paste0(
                ...
                'https://www.ncbi.nlm.nih.gov/books/NBK25501/ for details. ',
                'that using the provided url template will automatically ',
                'include the value from the `query_column` at the beginning of ',
                'the term.'
            ),
            value = '2008[pdat]',
            max_length = 1
