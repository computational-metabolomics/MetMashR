...
.annotation_venn_chart<-setClass(
    ...
    prototype = list(
        ...
        description=paste0(
            ...
            'annotation_sources.'),
        type="image",
        .params=c('factor_name','line_colour','fill_colour','labels','legend',
                  'group_column'),
          ...
          definition=function(obj,dobj,...) {
              ...
              if (length(L)>0) {
                  ...
                  if (length(obj$factor_name) != length(L)) {
                      stop('You must provide either a single factor_name ',
              ...
              # max 7(!) groups
              if (length(L)>7) {
                  stop('Venn chart can only be plotted for up to 7 groups. ',
...
#' @include annotation_source_class.R
#' @export 
struct_venn_set = function(
        set,
        ...) {
    
    sf::st_agr(set) = 'constant'
...
setMethod('+',c('struct_venn_set','struct_venn_set'),definition = function(e1,e2){
    e1@set = sf::st_union(e1@set,e2@set)
    sf::st_agr(e1@set)='constant'
    return(e1)
})

setMethod('-',c('struct_venn_set','struct_venn_set'),definition = function(e1,e2){
    e1@set = sf::st_difference(e1@set,e2@set)
    sf::st_agr(e1@set)='constant'
    return(e1)
})

setMethod('&',c('struct_venn_set','struct_venn_set'),definition = function(e1,e2){
    e1@set = sf::st_intersection(e1@set,e2@set)
    sf::st_agr(e1@set)='constant'
    return(e1)
})

setMethod('|',c('struct_venn_set','struct_venn_set'),definition = function(e1,e2){
    e1@set = sf::st_sym_difference(e1@set,e2@set)
    sf::st_agr(e1@set)='constant'
    return(e1)
})
