...
.filter_range <- setClass(
    ...
    prototype = list(
        ...
        description = paste0(
            "Removes annotations where the names column is greater than an ",
            "upper limit or less than a lower limit."
        ),
        type = "univariate",
        predicted = "filtered",
        ...
        upper_limit = entity(
            name = "Upper limit",
            description = paste0(
                "The upper limit used for filtering. Can be a value, or a ",
                "function that computes a value (e.g. mean)."
            ),
        ...
        lower_limit = entity(
            name = "Lower limit",
            description = paste0(
                "The lower limit used for filtering. Can be a value, or a ",
                "function that computes a value (e.g. mean)."
            ),
            type = c("numeric", "integer", "function"),
            value = -Inf,
        ...
        equal_to = entity(
            ...
            description = c(
                "TRUE" = paste0(
                    "greater/less than or equal to the limits are excluded."
                ),
                "FALSE" = paste0(
                    "greater/less than the limits are excluded."
                )
            ),
            value = TRUE,
            type = "logical",
...
setMethod(
    ...
    definition = function(M, D) {
        ...
        if (M$equal_to) {
            w <- which(X[[M$column_name]] <= lower_limit |
                X[[M$column_name]] >= upper_limit)
        } else {
            w <- which(X[[M$column_name]] < lower_limit |
                X[[M$column_name]] > upper_limit)
        }
        ...
        OUT[w] <- TRUE
        flags <- data.frame(
            flags = OUT, value = X[[M$column_name]],
            lower_limit = lower_limit,
            upper_limit = upper_limit
        )
