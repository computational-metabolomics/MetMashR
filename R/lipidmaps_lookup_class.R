...
lipidmaps_lookup = function(
    ...
    allowed=list(
        ...
        gene = c('gene_id','lmp_id','gene_name','gene_symbol','gene_synonyms',
                 'chromosome','map_location','summary','taxid','species',
                 'species_long','all'),
        protein = c('uniprot_id','lmp_id','gene_id','gene_name','gene_symbol',
    ...
    if (length(output_item)>1) {
        if (!all(output_item %in% allowed[[context]])) {
            stop('An output_item is not valid for the current context. ',
                 'See https://lipidmaps.org/resources/rest')
        }
...
.lipidmaps_lookup<-setClass(
    ...
    prototype=list(
        ...
        url_template = entity(
            ...
            value = paste0('<base_url>/<context>/<context_item>/',
        ...
        context=enum(
            name = 'Search context',
            description = paste0(
                'The search API context. Must be one of "compound", "gene", or ',
                '"protein"'),
            type='character',
            max_length = 1,
            value='compound',
