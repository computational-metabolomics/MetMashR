...
# Introduction
`r Biocpkg("MetMashR")` is an R package designed to facilitate the cleaning, filtering and 
combining of annotations from different sources. `r Biocpkg("MetMashR")` defines an
"annotation source" as a piece of software, proprietary or otherwise, that
...
library. `r Biocpkg("MetMashR")` was primarily designed for use with metabolomics data 
measured
by LCMS (hence "metabolite" in the package name) but could be extended to include
...
All of the objects defined in `r Biocpkg("MetMashR")` use or extend the class templates 
defined by the `r Biocpkg("struct")` package. Although originally intended for statistics
applications, the templates in the `r Biocpkg("struct")` package have proven to be adaptable
to many different scenarios and types of analysis/workflow step.

The use of `r Biocpkg("struct")` templates allows workflow steps to be applied in sequence 
...
# Getting Started
The latest versions of `r Biocpkg("struct")` and `r Biocpkg("MetMashR")` that are compatible 
with your 
...
`annotation_source` objects are the dataset used by all `r Biocpkg("MetMashR")` workflow 
steps. If you have used our `r Biocpkg("structToolbox")` package before, then annotation 
...
A `annotation_table` is defined by us as a `data.frame` of metabolite annotations
...
**Note:**  
It is not the aim of `r Biocpkg("MetMashR")` to generate these annotations. Instead we aim to
...
WF = 
    ...
    # step 4 query lipidmaps api for inchikey using the names provided by LipidSearch
    lipidmaps_lookup(
        query_column = 'LipidName',
        context = 'compound',
        context_item = 'abbrev',
    ...
    # step 5 query lipidmaps api for inchikey using the names provided by LipidSearch
    lipidmaps_lookup(
...
.DT(predicted(WF)$data[,c('LipidName','normalised_name','inchi_key_LipidName','inchi_key_normalised')])
```
...
.DT(predicted(CR)$data[,c('LipidName','normalised_name','inchi_key_normalised','count')])
```
